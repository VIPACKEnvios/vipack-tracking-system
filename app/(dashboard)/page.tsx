...
export default function DashboardPage() {
  ...
  const [paquetesUSA, setPaquetesUSA] = useState<PaqueteUSAAdmin[]>([]);
  const [cargandoPaquetesUSA, setCargandoPaquetesUSA] = useState(true);
  const [errorPaquetesUSA, setErrorPaquetesUSA] = useState("");
  const [busquedaPaquetesUSA, setBusquedaPaquetesUSA] = useState("");
  const [filtroPaquetesUSA, setFiltroPaquetesUSA] = useState<
    "todos" | "registrados" | "recibidos"
  >("todos");
  const [recibiendoPaqueteId, setRecibiendoPaqueteId] = useState("");
  const [resumenPaquetesUSA, setResumenPaquetesUSA] = useState<ResumenPaquetesUSA>({
    total: 0,
    registrados: 0,
    recibidos: 0,
  });
  ...
  async function cargarPaquetesUSA() {
    try {
      setCargandoPaquetesUSA(true);
      setErrorPaquetesUSA("");
      ...
      if (!respuesta.ok || !resultado.success) {
        throw new Error(
          resultado.error || "No fue posible cargar los paquetes USA."
        );
      }

      setPaquetesUSA(resultado.paquetes || []);
      setResumenPaquetesUSA(
        resultado.resumen || { total: 0, registrados: 0, recibidos: 0 }
      );
    } catch (error) {
      console.error("Error cargando Paquetes USA:", error);
      setPaquetesUSA([]);
      setResumenPaquetesUSA({ total: 0, registrados: 0, recibidos: 0 });
      setErrorPaquetesUSA(
        error instanceof Error
          ? error.message
          : "No fue posible cargar los paquetes USA."
      );
    } finally {
      setCargandoPaquetesUSA(false);
    }
  }

  async function marcarPaqueteRecibido(paquete: PaqueteUSAAdmin) {
    if (paquete.estatus.trim().toLowerCase() === "recibido") return;

    const confirmar = window.confirm(
      `¿Marcar como recibido el paquete ${paquete.rastreo} de ${paquete.cliente}?`
    );

    if (!confirmar) return;

    try {
      setRecibiendoPaqueteId(paquete.id);
      setErrorPaquetesUSA("");
      ...
      if (!respuesta.ok || !resultado.success) {
        throw new Error(
          resultado.error || "No fue posible marcar el paquete como recibido."
        );
      }

      await cargarPaquetesUSA();
    } catch (error) {
      console.error("Error recibiendo paquete USA:", error);
      const mensaje =
        error instanceof Error
          ? error.message
          : "No fue posible marcar el paquete como recibido.";
      setErrorPaquetesUSA(mensaje);
      alert(mensaje);
    } finally {
      setRecibiendoPaqueteId("");
    }
  }
  ...
  const paquetesUSAFiltrados = useMemo(() => {
    const termino = normalizarTexto(busquedaPaquetesUSA);

    return paquetesUSA.filter((paquete) => {
      const recibido = paquete.estatus.trim().toLowerCase() === "recibido";

      if (filtroPaquetesUSA === "registrados" && recibido) return false;
      if (filtroPaquetesUSA === "recibidos" && !recibido) return false;

      if (!termino) return true;

      return normalizarTexto(
        `${paquete.rastreo} ${paquete.cliente} ${paquete.tienda}`
      ).includes(termino);
    });
  }, [paquetesUSA, busquedaPaquetesUSA, filtroPaquetesUSA]);
  ...
  return (
    ...
    <div className="min-h-[calc(100vh-4rem)] bg-gradient-to-br from-slate-100 via-white to-blue-50 p-5 md:p-8">
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <section className="mt-8 overflow-hidden rounded-3xl border border-sky-200 bg-white shadow-lg">
          <div className="bg-gradient-to-r from-[#061b48] via-[#073b88] to-[#00a7a7] px-5 py-6 text-white md:px-7">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <p className="text-xs font-black uppercase tracking-[0.18em] text-cyan-100">
                  Recepción de mercancía
                </p>
                <h2 className="mt-1 text-2xl font-black md:text-3xl">📦 Paquetes USA</h2>
                <p className="mt-1 text-sm font-medium text-cyan-50">
                  Busca por rastreo, cliente o tienda y marca los paquetes cuando lleguen a VIPACK.
                </p>
              </div>

              <button
                type="button"
                onClick={cargarPaquetesUSA}
                disabled={cargandoPaquetesUSA}
                className="rounded-xl border border-white/30 bg-white/15 px-4 py-2.5 text-sm font-black text-white shadow-sm backdrop-blur transition hover:bg-white/25 disabled:cursor-not-allowed disabled:opacity-60"
              >
                {cargandoPaquetesUSA ? "Actualizando..." : "↻ Actualizar"}
              </button>
            </div>
          </div>
          ...
          <div className="p-4 md:p-6">
            <div className="grid grid-cols-3 gap-2 md:gap-4">
              <button
                type="button"
                onClick={() => setFiltroPaquetesUSA("todos")}
                className={`rounded-2xl border p-3 text-left transition md:p-4 ${
                  filtroPaquetesUSA === "todos"
                    ? "border-blue-400 bg-blue-50 ring-2 ring-blue-100"
                    : "border-slate-200 bg-slate-50 hover:bg-slate-100"
                }`}
              >
                <p className="text-[11px] font-black uppercase tracking-wide text-slate-500 md:text-xs">Total</p>
                <p className="mt-1 text-2xl font-black text-slate-950 md:text-3xl">
                  {cargandoPaquetesUSA ? "..." : resumenPaquetesUSA.total}
                </p>
              </button>

              <button
                type="button"
                onClick={() => setFiltroPaquetesUSA("registrados")}
                className={`rounded-2xl border p-3 text-left transition md:p-4 ${
                  filtroPaquetesUSA === "registrados"
                    ? "border-blue-400 bg-blue-50 ring-2 ring-blue-100"
                    : "border-blue-200 bg-blue-50/50 hover:bg-blue-50"
                }`}
              >
                <p className="text-[11px] font-black uppercase tracking-wide text-blue-700 md:text-xs">Por recibir</p>
                <p className="mt-1 text-2xl font-black text-blue-800 md:text-3xl">
                  {cargandoPaquetesUSA ? "..." : resumenPaquetesUSA.registrados}
                </p>
              </button>

              <button
                type="button"
                onClick={() => setFiltroPaquetesUSA("recibidos")}
                className={`rounded-2xl border p-3 text-left transition md:p-4 ${
                  filtroPaquetesUSA === "recibidos"
                    ? "border-emerald-400 bg-emerald-50 ring-2 ring-emerald-100"
                    : "border-emerald-200 bg-emerald-50/50 hover:bg-emerald-50"
                }`}
              >
                <p className="text-[11px] font-black uppercase tracking-wide text-emerald-700 md:text-xs">Recibidos</p>
                <p className="mt-1 text-2xl font-black text-emerald-800 md:text-3xl">
                  {cargandoPaquetesUSA ? "..." : resumenPaquetesUSA.recibidos}
                </p>
              </button>
            </div>

            <div className="mt-5">
              <label htmlFor="buscar-paquete-usa" className="mb-2 block text-sm font-black text-slate-700">
                Buscar paquete
              </label>
              <input
                id="buscar-paquete-usa"
                type="search"
                value={busquedaPaquetesUSA}
                onChange={(event) => setBusquedaPaquetesUSA(event.target.value)}
                placeholder="Rastreo, cliente o tienda..."
                className="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm font-semibold text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-blue-500 focus:ring-4 focus:ring-blue-100"
              />
            </div>
            ...
            {cargandoPaquetesUSA ? (
              <div className="mt-5 rounded-2xl bg-slate-50 p-8 text-center font-semibold text-slate-500">
                Cargando paquetes USA...
              </div>
            ) : paquetesUSAFiltrados.length === 0 ? (
              <div className="mt-5 rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-8 text-center">
                <p className="font-black text-slate-700">No hay paquetes para mostrar.</p>
              ...
              <>
                <div className="mt-5 space-y-3 md:hidden">
                  {paquetesUSAFiltrados.map((paquete) => {
                    const recibido = paquete.estatus.trim().toLowerCase() === "recibido";
                    const procesando = recibiendoPaqueteId === paquete.id;

                    return (
                      <article key={paquete.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            <p className="truncate text-sm font-black text-slate-950">{paquete.cliente || "Sin cliente"}</p>
                            <p className="mt-1 break-all font-mono text-sm font-black text-blue-700">{paquete.rastreo}</p>
                          </div>
                          <span className={`shrink-0 rounded-full px-3 py-1 text-xs font-black ${
                            recibido ? "bg-emerald-100 text-emerald-800" : "bg-blue-100 text-blue-800"
                          }`}>
                            {recibido ? "✓ Recibido" : "Registrado"}
                          </span>
                        </div>

                        <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
                          <div className="rounded-xl bg-slate-50 p-2.5">
                            <p className="font-bold text-slate-500">Tienda</p>
                            <p className="mt-0.5 font-black text-slate-800">{paquete.tienda || "—"}</p>
                          </div>
                          <div className="rounded-xl bg-slate-50 p-2.5">
                            <p className="font-bold text-slate-500">Registro</p>
                            <p className="mt-0.5 font-black text-slate-800">{paquete.fechaRegistro || "—"}</p>
                          </div>
                        </div>

                        {recibido ? (
                          <div className="mt-3 rounded-xl bg-emerald-50 px-3 py-2 text-sm font-black text-emerald-800">
                            Recibido: {paquete.fechaRecibido || "Fecha registrada"}
                          </div>
                        ) : (
                          <button
                            type="button"
                            onClick={() => marcarPaqueteRecibido(paquete)}
                            disabled={procesando}
                            className="mt-3 w-full rounded-xl bg-emerald-600 px-4 py-3 text-sm font-black text-white shadow-sm transition hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-60"
                          >
                            {procesando ? "Guardando..." : "✓ Recibir paquete"}
                          </button>
                        )}
                      </article>
                    );
                ...
                <div className="mt-5 hidden overflow-x-auto rounded-2xl border border-slate-200 md:block">
                  <table className="min-w-full divide-y divide-slate-200 text-sm">
                    <thead className="bg-slate-50">
                      <tr>
                        <th className="px-4 py-3 text-left font-black text-slate-600">Cliente</th>
                        <th className="px-4 py-3 text-left font-black text-slate-600">Rastreo</th>
                        <th className="px-4 py-3 text-left font-black text-slate-600">Tienda</th>
                        <th className="px-4 py-3 text-left font-black text-slate-600">Registro</th>
                        <th className="px-4 py-3 text-left font-black text-slate-600">Estado</th>
                        <th className="px-4 py-3 text-right font-black text-slate-600">Acción</th>
                      </tr>
                    </thead>
                      ...
                      {paquetesUSAFiltrados.map((paquete) => {
                        ...
                        return (
                          <tr key={paquete.id} className="hover:bg-slate-50">
                            <td className="px-4 py-3 font-bold text-slate-900">{paquete.cliente || "Sin cliente"}</td>
                            <td className="px-4 py-3 font-mono font-black text-blue-700">{paquete.rastreo}</td>
                            <td className="px-4 py-3 font-semibold text-slate-700">{paquete.tienda || "—"}</td>
                            <td className="px-4 py-3 font-semibold text-slate-600">{paquete.fechaRegistro || "—"}</td>
                            <td className="px-4 py-3">
                              <span className={`inline-flex rounded-full px-3 py-1 text-xs font-black ${
                                recibido ? "bg-emerald-100 text-emerald-800" : "bg-blue-100 text-blue-800"
                              }`}>
                                {recibido ? `✓ Recibido${paquete.fechaRecibido ? ` · ${paquete.fechaRecibido}` : ""}` : "Registrado"}
                              </span>
                            </td>
                            <td className="px-4 py-3 text-right">
                              {recibido ? (
                                <span className="font-black text-emerald-700">Listo</span>
                              ) : (
                                <button
                                  type="button"
                                  onClick={() => marcarPaqueteRecibido(paquete)}
                                  disabled={procesando}
                                  className="rounded-xl bg-emerald-600 px-4 py-2 text-xs font-black text-white shadow-sm transition hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-60"
                                >
                                  {procesando ? "Guardando..." : "✓ Recibir"}
                                </button>
                              )}
                            </td>
                          </tr>
