...
export async function GET() {
  try {
    ...
    /*
     * 3. Buscar la carpeta Envios
     * directamente por ruta
     */
    const enviosResponse = await fetch(
      "https://graph.microsoft.com/v1.0/me/drive/root:/Envios:/children?$select=id,name,folder,file,webUrl,parentReference,lastModifiedDateTime,size",
      {
        headers: {
          Authorization:
            `Bearer ${accessToken}`,
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            "No se pudo abrir la carpeta Envios.",
          detalle:
            enviosData,
        },
        { status: 400 }
