        ...
        <section className="relative mx-auto -mt-4 max-w-6xl px-3.5 pb-10 sm:-mt-10 sm:px-8">
          ...
          {otrosArchivos.length > 0 && (
            ...
            <div className="mt-5 space-y-5 sm:mt-8">
              <div className="rounded-[24px] border border-slate-200 bg-white p-4 shadow-lg shadow-slate-200/40 sm:p-7">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="text-xs font-black uppercase tracking-[0.12em] text-blue-600">Compras en Estados Unidos</p>
                    <h2 className="mt-1 text-2xl font-black text-slate-900 sm:text-4xl">Paquetes USA</h2>
                    <p className="mt-2 max-w-2xl text-sm font-medium leading-6 text-slate-500">Registra aquí cada compra que llegará a VIPACK. Tu cliente se identifica automáticamente desde este inventario.</p>
                  </div>
                  <div className="rounded-2xl bg-blue-50 p-3 text-[#0a3183]"><IconoCaja className="h-8 w-8" /></div>
                </div>

                <div className="mt-5 grid grid-cols-3 gap-2 sm:gap-4">
                  <div className="rounded-2xl bg-slate-50 p-3 text-center"><p className="text-2xl font-black text-slate-900">{paquetes.length}</p><p className="text-[10px] font-black uppercase text-slate-500">Total</p></div>
                  <div className="rounded-2xl bg-blue-50 p-3 text-center"><p className="text-2xl font-black text-blue-700">{paquetesRegistrados}</p><p className="text-[10px] font-black uppercase text-blue-600">Registrados</p></div>
                  <div className="rounded-2xl bg-emerald-50 p-3 text-center"><p className="text-2xl font-black text-emerald-700">{paquetesRecibidos}</p><p className="text-[10px] font-black uppercase text-emerald-600">Recibidos</p></div>
                </div>
              </div>

              <form onSubmit={registrarPaquete} className="rounded-[24px] border border-slate-200 bg-white p-4 shadow-lg shadow-slate-200/40 sm:p-7">
                <h3 className="text-xl font-black text-slate-900">Registrar paquete</h3>
                <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-3">
                  <label className="block"><span className="text-sm font-black text-slate-700">Número de rastreo</span><input value={formPaquete.rastreo} onChange={(e) => setFormPaquete((a) => ({ ...a, rastreo: e.target.value }))} required autoComplete="off" className="mt-2 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3.5 text-base font-semibold outline-none focus:border-blue-400 focus:bg-white" placeholder="Ej. 1Z..." /></label>
                  <label className="block"><span className="text-sm font-black text-slate-700">Tienda</span><input value={formPaquete.tienda} onChange={(e) => setFormPaquete((a) => ({ ...a, tienda: e.target.value }))} required className="mt-2 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3.5 text-base font-semibold outline-none focus:border-blue-400 focus:bg-white" placeholder="Amazon, Walmart..." /></label>
                  <label className="block"><span className="text-sm font-black text-slate-700">Fecha de compra (opcional)</span><input type="date" value={formPaquete.fechaCompra} onChange={(e) => setFormPaquete((a) => ({ ...a, fechaCompra: e.target.value }))} className="mt-2 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3.5 text-base font-semibold outline-none focus:border-blue-400 focus:bg-white" /></label>
                </div>
                {errorPaquetes && <div className="mt-4 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-bold text-red-700">{errorPaquetes}</div>}
                {mensajePaquetes && <div className="mt-4 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-bold text-emerald-700">{mensajePaquetes}</div>}
                <button type="submit" disabled={guardandoPaquete} className="mt-4 w-full rounded-2xl bg-[#0a3183] px-5 py-3.5 text-sm font-black text-white shadow-md transition hover:bg-[#08286b] disabled:cursor-not-allowed disabled:opacity-60 sm:w-auto">{guardandoPaquete ? "Guardando..." : "+ Registrar paquete"}</button>
              </form>

              <div className="rounded-[24px] border border-slate-200 bg-white p-4 shadow-lg shadow-slate-200/40 sm:p-7">
                <div className="flex items-center justify-between gap-3"><h3 className="text-xl font-black text-slate-900">Mis paquetes</h3><button type="button" onClick={() => void cargarPaquetes()} disabled={cargandoPaquetes} className="rounded-xl bg-blue-50 px-3 py-2 text-xs font-black text-[#0a3183] disabled:opacity-50">Actualizar</button></div>
                {cargandoPaquetes ? <div className="py-10 text-center text-sm font-bold text-slate-500">Cargando paquetes...</div> : paquetes.length === 0 ? <div className="mt-4 rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-8 text-center text-sm font-semibold text-slate-500">Aún no hay paquetes registrados.</div> : (
                  <>
                    <div className="mt-4 space-y-3 md:hidden">{paquetes.map((paquete) => <article key={paquete.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-xs font-black uppercase text-slate-400">Rastreo</p><p className="mt-1 break-all text-base font-black text-slate-900">{paquete.rastreo}</p></div><span className={`shrink-0 rounded-full px-2.5 py-1 text-[10px] font-black uppercase ${paquete.estatus.trim().toLowerCase() === "recibido" ? "bg-emerald-100 text-emerald-700" : "bg-blue-100 text-blue-700"}`}>{paquete.estatus || "Registrado"}</span></div><div className="mt-4 grid grid-cols-2 gap-3 text-sm"><div><p className="text-xs font-bold text-slate-400">Tienda</p><p className="font-bold text-slate-700">{paquete.tienda || "—"}</p></div><div><p className="text-xs font-bold text-slate-400">Compra</p><p className="font-bold text-slate-700">{paquete.fechaCompra || "—"}</p></div><div><p className="text-xs font-bold text-slate-400">Registro</p><p className="font-bold text-slate-700">{paquete.fechaRegistro || "—"}</p></div><div><p className="text-xs font-bold text-slate-400">Recibido</p><p className="font-bold text-slate-700">{paquete.fechaRecibido || "Pendiente"}</p></div></div></article>)}</div>
