  ...
  for (const fileName of Object.keys(zip.files)) {
    ...
    if (paqueteria === "DHL") {
      ...
      for (const match of matchesDHL) {
        ...
        if (guia.length >= 8 && guia.length <= 30) {
          guias.push(guia);
        }
      }
    }
    ...
    if (paqueteria === "FEDEX") {
      const posiblesFedex =
        textoPDF.match(/\b\d{12}\b|\b\d{15}\b/g) || [];
...
const clientesConPDF = (jsonData as any[]).map((cliente) => {
  ...
  const pdfEncontrado = nombresPDF.find((pdf) => {
    ...
    return nombrePDF.includes(nombreCliente) || nombreCliente.includes(nombrePDF);
  });
...
alert("Excel y ZIP validados correctamente");
 ...
 const handleSendWhatsApp = async () => {
  ...
  for (const cliente of clientes) {
    if (
      cliente.enviado &&
      String(cliente.enviado).toUpperCase() === "SI"
    ) {
    ...
    const pdfUrl =
      `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/guias/${safePdfName}`;
    ...
    if (data.success) {
      ...
      nuevosLogs.push(`✅ WhatsApp enviado a ${cliente.cliente}`);
    } else {
...
>
  ...
  <div>
    <h1 className="text-5xl font-extrabold text-[#072c74]">
